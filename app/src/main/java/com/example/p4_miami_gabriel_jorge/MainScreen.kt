    ...
    Scaffold(
        ...
        Box(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
            LazyVerticalStaggeredGrid(
                    ...
                    contentPadding =
                            PaddingValues(start = 12.dp, end = 12.dp, top = 12.dp, bottom = 100.dp),
                ...
                ) { item -> MiamiCard(item = item, onClick = { onItemClick(item.id) }) }
            }
...
fun MiamiCard(item: MiamiItem, onClick: () -> Unit) {
    Card(
            modifier =
                    Modifier.fillMaxWidth().aspectRatio(item.aspectRatio).padding(4.dp).clickable {
                        onClick()
                    },
